...
export default function HealthMonitorPage() {
  ...
  const getVitalStatus = (vital: any, value: any) => {
    switch (vital) {
      case "heartRate":
  ...
  if (loading) {
    return (
      <div className="p-6">
        <div className="text-white">Đang tải dữ liệu bệnh nhân...</div>
      </div>
    )
  }
          ...
          >
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-neutral-400">
                    {patient.patient_code} • Phòng {patient.room_number} • Giường {patient.bed_number} • {patient.age} tuổi
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Badge className={getStatusColor(patient.status)}>
                    {patient.status === "stable" ? "ỔN ĐỊNH" : patient.status === "warning" ? "CẢNH BÁO" : patient.status === "critical" ? "NGUY HIỂM" : "KHÔNG XÁC ĐỊNH"}
                  </Badge>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="text-xs text-neutral-400">
                <span className="text-white">{patient.condition || "Chưa có chẩn đoán"}</span>
                {/* Có thể thêm thông tin đồng bộ nếu có */}
              </div>

              {/* Vital Signs Grid - dữ liệu mẫu */}
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-lg font-bold text-white font-mono">78 <span className="text-xs text-neutral-400">bpm</span></div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="text-lg font-bold text-white font-mono">98<span className="text-xs text-neutral-400">%</span></div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="text-lg font-bold text-white font-mono">36.8<span className="text-xs text-neutral-400">°C</span></div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="text-lg font-bold text-white font-mono">130/85</div>
                </div>
              </div>
              {/* Các chỉ số khác cũng để mẫu hoặc placeholder */}

              {/* Additional Metrics */}
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div className="text-center">
                  <div className="text-neutral-400">Bước chân</div>
                  <div className="text-white font-mono">2450</div>
                </div>
                <div className="text-center">
              ...
              {patient.alerts && patient.alerts.length > 0 && (
                <div className="space-y-1">
                  {patient.alerts.map((alert: any, index: any) => (
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <Card className="bg-neutral-900 border-neutral-700 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-xl font-bold text-white tracking-wider">{(selectedPatient as any).name}</CardTitle>
                <p className="text-sm text-neutral-400">
                  {(selectedPatient as any).patient_code} • Phòng {(selectedPatient as any).room_number} • Giường {(selectedPatient as any).bed_number} • {(selectedPatient as any).condition}
                </p>
              </div>
              <Button
                variant="ghost"
                      ...
                      {[
                        {
                          icon: Heart,
                          label: "Nhịp tim",
                          value: `${(selectedPatient as any).vitals.heartRate} bpm`,
                          color: "text-red-500",
                        },
                        {
                          icon: Droplets,
                          label: "SpO2",
                          value: `${(selectedPatient as any).vitals.spO2}%`,
                          color: "text-blue-500",
                        },
                        {
                          icon: Thermometer,
                          label: "Nhiệt độ",
                          value: `${(selectedPatient as any).vitals.temperature}°C`,
                          color: "text-orange-500",
                        },
                        {
                          icon: Activity,
                          label: "Huyết áp",
                          value: (selectedPatient as any).vitals.bloodPressure,
                          color: "text-white",
                        },
                      ].map((vital, index) => (
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-2 bg-neutral-800 rounded">
                        ...
                        <span className="text-sm text-white font-mono">
                          {(selectedPatient as any).vitals.steps.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex items-center justify-between p-2 bg-neutral-800 rounded">
                        <span className="text-sm text-neutral-300">Chất lượng giấc ngủ</span>
                        <span className="text-sm text-white font-mono">{(selectedPatient as any).vitals.sleepQuality}%</span>
                      </div>
                      <div className="flex items-center justify-between p-2 bg-neutral-800 rounded">
                        <span className="text-sm text-neutral-300">Mức độ stress</span>
                        <span className="text-sm text-white font-mono">{(selectedPatient as any).vitals.stressLevel}%</span>
                      </div>
                    </div>
                  </div>
                </div>
